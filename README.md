...
일반 텍스처나 빌보드 텍스처를 이용하여 테스트해봤으나,   
위치, 간격을 구현하는 코드가 복잡하고 무엇보다도 카메라의 상대적 위치에 따라 간격의 변화 정도가 심각할 정도로 왜곡이 되었습니다.  

이 문제를 해결하기 위해서 고민하던 중 UI만큼은 화면에서 절대적 위치에 보여지는 점을 착안해 UI로 아이콘을 구현했습니다.
